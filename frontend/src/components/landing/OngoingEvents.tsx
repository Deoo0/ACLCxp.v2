...
function EventCard({ event, destination, action }: { event: Row; destination: string; action: string }) {
  ...
  return <article className="group flex min-w-0 flex-col overflow-hidden rounded-2xl border border-white/10 bg-[#141618] transition duration-300 hover:border-amber-300/35 hover:shadow-xl hover:shadow-black/25">
    <div className="flex items-center justify-between gap-3 border-b border-white/10 px-5 py-3">
      ...
      <span className="flex shrink-0 items-center gap-1.5 text-[10px] font-semibold uppercase tracking-wider text-amber-200"><span className="h-1.5 w-1.5 rounded-full bg-amber-300" />Upcoming</span>
    </div>
    <div className="relative isolate flex aspect-[4/3] items-center justify-center overflow-hidden bg-neutral-950">
      {hasPhoto ? <>
        ...
        <a href={photo} target="_blank" rel="noopener noreferrer" className="flex h-full w-full items-center justify-center p-3 focus-visible:outline-2 focus-visible:outline-offset-[-3px] focus-visible:outline-amber-300" aria-label={`View full poster for ${String(event.title)}`}>
          ...
          <span className="absolute bottom-3 right-3 rounded-full border border-white/20 bg-black/70 p-2 text-white"><ArrowUpRight className="h-3.5 w-3.5" /></span>
        </a>
...
export default function OngoingEvents() {
  const query = useApi<PageData>("/events/?status=PUBLISHED&page_size=3");
  const { user } = useAuth();
