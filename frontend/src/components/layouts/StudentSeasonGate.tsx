...
export default function StudentSeasonGate({ children }: { children: ReactNode }) {
  ...
  return <main className="flex min-h-screen items-center justify-center bg-neutral-950 px-5 py-12 text-white">
    <section className="w-full max-w-lg rounded-3xl border border-white/10 bg-gradient-to-br from-amber-300/10 to-neutral-900 p-7 sm:p-10">
      ...
      <p className="mt-4 text-sm leading-6 text-neutral-400">{season?.status === 'CLOSED' ? 'This season has ended. Your account stays available, but your student workspace is locked until you activate a ticket for the next season.' : enrolled ? 'Your ticket is activated. Your fresh student dashboard will unlock when the administrator starts this season.' : 'Keep your existing account. Enter a new ticket for this season to activate your participation.'}</p>
