    ...
    <div className="relative z-10 mx-auto flex min-h-[calc(100vh-2.5rem)] max-w-6xl flex-col">
      ...
      <main className="flex flex-1 items-center justify-center py-10"><div className="grid w-full max-w-4xl overflow-hidden rounded-2xl border border-white/10 bg-neutral-900/60 shadow-2xl shadow-black/30 md:grid-cols-[.82fr_1.18fr]">
        ...
        <div className="auth-panel-enter px-4 py-7 sm:px-12 sm:py-12"><div className="mx-auto max-w-sm"><div className="md:hidden"><p className="text-xs font-semibold uppercase tracking-[.16em] text-amber-400">ACLCxp Student Portal</p><h1 className="mt-2 text-2xl font-semibold text-neutral-50">Sign in</h1></div><div className="hidden md:block"><h2 className="text-2xl font-semibold text-neutral-50">Welcome back</h2><p className="mt-2 text-sm text-neutral-400">Enter your account details to continue.</p></div>
          ...
          <form onSubmit={handleLogin} className="mt-7 space-y-5"><div><label htmlFor="student-number" className="mb-2 block text-sm font-medium text-neutral-300">Student Number</label><div className="relative"><FaUser className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-neutral-400" /><input id="student-number" type="text" autoCapitalize="characters" spellCheck={false} autoComplete="username" value={studentId} onChange={(e) => setStudentId(e.target.value.replace(/[^a-zA-Z0-9-]/g, "").toUpperCase())} placeholder="e.g. 2026-12345" maxLength={20} className="w-full rounded-xl border border-white/10 bg-neutral-950/50 py-2.5 pl-10 pr-3 text-neutral-100 placeholder:text-neutral-600 outline-none transition focus:border-amber-400/70 focus:ring-2 focus:ring-amber-400/15" /></div></div>
            <div><div className="mb-2 flex items-center justify-between"><label htmlFor="password" className="text-sm font-medium text-neutral-300">Password</label><button type="button" id="forgot-password-trigger" onClick={() => setShowForgotModal(true)} className="text-xs font-medium text-amber-400 hover:text-amber-300">Forgot password?</button></div><div className="relative"><FaLock className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-neutral-400" /><input id="password" type={showPassword ? "text" : "password"} autoComplete="current-password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Enter your password" maxLength={128} className="w-full rounded-xl border border-white/10 bg-neutral-950/50 py-2.5 pl-10 pr-11 text-neutral-100 placeholder:text-neutral-600 outline-none transition focus:border-amber-400/70 focus:ring-2 focus:ring-amber-400/15" /><button type="button" onClick={() => setShowPassword((visible) => !visible)} aria-label={showPassword ? "Hide password" : "Show password"} className="absolute right-2 top-1/2 -translate-y-1/2 flex h-11 w-11 items-center justify-center rounded-lg text-neutral-400 transition hover:bg-white/[0.05] hover:text-neutral-300">{showPassword ? <FaEyeSlash /> : <FaEye />}</button></div></div>
          ...
          </form><p className="mt-7 border-t border-white/10 pt-5 text-center text-sm text-neutral-400">Need an account? <Link to="/register" className="font-semibold text-amber-400 hover:text-amber-300">Activate your account</Link></p></div></div>
      </div></main>
